...
static void getMetrEvent(buf_len_t len)
{
	if (len >0)
	{
...
static void setMetrEvent(buf_len_t len)
{
	if (len >0)
	{
...
static void RunCmd(void)
{
	switch (Com.buf[CMD_POS])
	{
		...
		case CMD_WRITE_EE:
		{
			...
			if (from == 1024)
			{
				if (Ind.ready()) Ind.setMetr(dptr, n, &setMetrEvent);
			}
		}
		break;
...
//            
			...
			switch (workData.AppState)// & 0b00111111)
			{
				...
				break;					
				case APP_WORK:					
					if (TestModeTimer == 0 && Ind.ready()) {Ind.getData(&WorkEvent);} //TODO: remove test regim
					else WorkEvent(1);
					//WorkEvent(0);
				break;
			}
