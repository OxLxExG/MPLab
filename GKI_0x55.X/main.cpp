...
#define SWAP(x,y) tmp = y; x.B[1] = tmp; x.B[0] = (uint16_t) tmp >> 8
...
static void UpdateWorkData(void)
{
    ...
    {
        cmd_0x55_t* p = (cmd_0x55_t*) &Com.buf[0];
        int16_t tmp;

        SWAP(p->T, workData.dat.T);
        SWAP(p->ax, workData.dat.accel.X);
        SWAP(p->ay, workData.dat.accel.Y);
        SWAP(p->az, workData.dat.accel.Z);
        SWAP(p->hx, workData.dat.magnit.X);
        SWAP(p->hy, workData.dat.magnit.Y);
        SWAP(p->hz, workData.dat.magnit.Z);        
    }
}
