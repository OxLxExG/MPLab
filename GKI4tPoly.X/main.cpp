...
//#define dataprogT (*(TrrTemp_t*)0x8200)
...
#define SCALE_A 7700 // 7000;
#define SCALE_H 12000 // 14000;
#define SCALE_AMP 1000 
...
static void kosoAxis(TrrSensor_t* k, Dat_m_t& nt)
{
    Dat_m_t t = nt;    
    nt.X = t.X + t.Y*k->X[5]+t.Z*k->X[7];
    nt.Y = t.Y + t.X*k->Y[0]+t.Z*k->Y[7];
    nt.Z = t.Z + t.X*k->Z[0]+t.Y*k->Z[2];
}


static void trrAxisX(float* k, t4_t& t, Dat_m_t& nt, float& tr, bool koso)
{
    tr = nt.X*(k[0]+ k[1]*t.t1+ k[2]*t.t2+ k[3]*t.t3 + k[4]*t.t4)
       +       k[9]+k[10]*t.t1+k[11]*t.t2+k[12]*t.t3+k[13]*t.t4;
    if (!koso)
    {
       tr += nt.Y*(k[5]+ k[6]*t.t1)    
       + nt.Z*(k[7]+ k[8]*t.t1);            
    }
}
static void trrAxisY(float* k, t4_t& t, Dat_m_t& nt, float& tr, bool koso)
{
    tr = nt.Y*(k[2]+ k[3]*t.t1+ k[4]*t.t2+ k[5]*t.t3+ k[6]*t.t4)           
       +       k[9]+k[10]*t.t1+k[11]*t.t2+k[12]*t.t3+k[13]*t.t4;
    if (!koso)
    {
       tr += nt.X*(k[0]+ k[1]*t.t1)
       + nt.Z*(k[7]+ k[8]*t.t1);            
    }
}
static void trrAxisZ(float* k, t4_t& t, Dat_m_t& nt, float& tr, bool koso)
{
    tr = nt.Z*(k[4]+ k[5]*t.t1+k[6]*t.t2+k[7]*t.t3 + k[8]*t.t4)    
       +       k[9]+k[10]*t.t1+k[11]*t.t2+k[12]*t.t3+k[13]*t.t4;
    if (!koso)
    {
       tr +=  nt.X*(k[0]+ k[1]*t.t1)
...
#define MAGTX dataprog.Inclin.Poly.magnit
#define ACCTX dataprog.Inclin.Poly.accel
#define KNT dataprog.toNanoTesla.kmul
//#define TRRT  dataprogT
...
static void UpdateWorkData(void)
{
    ...
    t4_t t;
    t.t1 = (workData.dat.T-25)/100; 
    t.t2 = t.t1*t.t1; 
    t.t3 = t.t2*t.t1; 
    t.t4 = t.t3*t.t1; 
    
	Dat_m_t nt;
	nt = workData.dat.accel;
	nt.X = nt.X/SCALE_A;
	nt.Y = nt.Y/SCALE_A;
	nt.Z = nt.Z/SCALE_A;
    bool koso = ACCTX.Y[0] == 0;
	trrAxisX(ACCTX.X, t, nt, AX,koso);
	trrAxisY(ACCTX.Y, t, nt, AY,koso);
	trrAxisZ(ACCTX.Z, t, nt, AZ,koso);
    
    if (koso) kosoAxis(&ACCTX,nt);
	
	nt = workData.dat.magnit;
	nt.X = nt.X/SCALE_H;
	nt.Y = nt.Y/SCALE_H;
	nt.Z = nt.Z/SCALE_H;
	trrAxisX(MAGTX.X, t, nt, MX,koso);
	trrAxisY(MAGTX.Y, t, nt, MY,koso);
	trrAxisZ(MAGTX.Z, t, nt, MZ,koso);

    if (koso) kosoAxis(&MAGTX,nt);
	
	GTOT =hypot3D(workData.dat.accel_m)*10*SCALE_AMP;
	MTOT =hypot3D(workData.dat.magnit_m)*10*SCALE_AMP;
	...
	workData.dat.accel_m.X *= SCALE_AMP;
	workData.dat.accel_m.Y *= SCALE_AMP;
	workData.dat.accel_m.Z *= SCALE_AMP;
