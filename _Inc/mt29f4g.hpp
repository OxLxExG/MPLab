...
typedef enum {
    ...
    // критические ошибки        
//    NAND_BAD_RESET = 1 << 0,
//    NAND_NOT_FORMATED = 1 << 1,
    // LastReis может быть восстановлен неверно        
    NAND_BAD_CH_FORMAT_REI = 1 << 0,
    NAND_BAD_CH_FORMAT_BLO = 1 << 1,
    // при сбросе восстановленный waddr wreis CurrReisStartBlock
    // несовпали с eeprom, видимо начат новый рейс        
    NAND_BAD_EEPROM_CURSOR = 1 << 2,
    // просто информация что есть плохие блоки        
    NAND_BAD_BLOCK = 1 << 3,
    // ошибки восстановления кадра       
    NAND_BAD_RESTORE_1 = 1 << 4,
    NAND_BAD_RESTORE_2 = 1 << 5,
    NAND_BAD_RESTORE_3 = 1 << 6,

    // критическая ошибка чтения        
    NAND_BAD_REIS = 1 << 8,
    // if (rcol > PAGE_DATA_SIZE)        
    NAND_BAD_RCOL_1 = 1 << 9,
    NAND_BAD_RCOL_2 = 1 << 10,
    NAND_BAD_RCOL_3 = 1 << 11,
} nand_errors_t;

#define  NAND_BAD_CRITICAL (NAND_BAD_RESET | NAND_NOT_FORMATED | NAND_BAD_CH_FORMAT_REI | NAND_BAD_CH_FORMAT_BLO) 
...
public:
    ...
    uint32_t restore_read_kadr;
    uint32_t restore_expect_kadr;
    uint32_t restore_begin_addr; 
    uint8_t restore_cnt;
    ...
    bool BeginPageRead(RowAddrs* adr) {
     ...
     void CacheRead(loadCol* col, uint8_t* buf, uint16_t len) {
    ...
    //// 3. ошибки форматирования отсутствуют
    //// 4. LastReis найдена
    ...
    //INLN 
    bool Restore(lastReisInfo_t* lastEepromReis, uint16_t len, int32_t* lastKadr) {
        if (lastEepromReis->reis == LastReis.reis && lastEepromReis->startBlock == LastReis.startBlock) {
            ...
            // 1. адрес текущего (записываемого) блока //

            restore_begin_addr = (uint32_t) (LastReis.blocks - 1) * PAGE_DATA_SIZE * NUM_PAGES_PER_BLOCK;

            uint32_t kadr_0 = restore_begin_addr / len; 
            restore_expect_kadr = kadr_0+1;
            pageCnt = (kadr_0 * len) % PAGE_DATA_SIZE; 
            if (pageCnt > 0)
            {
                pageCnt = (pageCnt + len) % PAGE_DATA_SIZE; // next kadr in new block 
                restore_expect_kadr++;
            }
            ...
            while (restore_cnt <= NUM_PAGES_PER_BLOCK) {
                if (IsEmpty) {
                    unlockAllBlock();
                    // восстановить pageCnt waddr wreis и т.д.
                    *lastKadr = restore_expect_kadr-1;
                    ...
                    deinit();
                    if (!Res && Callback != NULL) Callback(NAND_BAD_RESTORE_3);
                    return Res;
                } else {
                    loadCol c = {pageCnt};
                    CacheRead(&c, (uint8_t*)&restore_read_kadr, 4);
                    pageCnt += len;
                    if (restore_read_kadr != restore_expect_kadr) 
                    {
            ...
            deinit();
            if (Callback != NULL) Callback(NAND_BAD_RESTORE_3);
            return false;
        } else {
    ...
    bool SetLastReis(uint16_t reis)
    {
        init();
        RowAddrs a = {.page = 0, .block = 0};
        uint16_t cnt = 0;
        reis_t cur;
        ...
        while (cnt < NUM_BLOCKS) {
            ...
            if (cur.reis == reis && cur.block == 0)
            {
                LastReis.reis = reis;
                LastReis.blocks = 1;
                LastReis.endBlock = a.block;
                LastReis.startBlock = a.block;
                cnt = 0;
                a.block++;
                while (cnt < NUM_BLOCKS) {
                    if (SkipBlock(&a, &cur)) {
    ...
    //    проверка на NAND на ошибки форматирования
    //    и заодно инициализация структуры LastReis 
...
//        }
    ...
    bool write(void* vbuf, buf_len_t cnt) {
        ...
        if (pageCnt + cnt >= PAGE_DATA_SIZE) {
            ...
            uint16_t pc = PAGE_DATA_SIZE - pageCnt;
...
public:
    ...
    INLN ram_t(void) {
        ...
        *port_pin_ctrl = PORT_ISC_INTDISABLE_gc | PORT_PULLUPEN_bm;
        ...
        SPI.CTRLB = SPI_MODE_3_gc
                | 1 << SPI_SSD_bp 
                ; // Slave Select Disable: disabled 		        
        deinit();
    }
    ...
    bool NAND_Reset(void) {
        ...
        bool r = waitRDY(20);
        deinit();
        //if (!r && Callback != NULL) Callback(NAND_BAD_RESET);
        return r;
    }
    ...
    bool CheckReis(lastReisInfo_t* lr) {
        ...
        bool res = lr->reis == wreis.reis && lr->startBlock == CurrReisStartBlock;
